...
```{r, echo = FALSE}
# Borrowed from googlesheets to prevent running vignette on CRAN
...
if (!file.exists(cps_ddi_file) | !file.exists(cps_data_file)) {
  stop(paste0(
    "Could not find CPS data.\n\nPlease make sure that the filenames are correct: ", 
    "\nddi - ", cps_ddi_file, "\ndata - ", cps_data_file, "\n\nAlso, that you are in ",
    "the correct directory if you are using a relative path:\nCurrent directory - ", 
    getwd(), " - is that where you saved the data?"
  ))
}
```
