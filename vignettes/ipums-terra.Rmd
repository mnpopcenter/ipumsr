...
if (!file.exists(micro_extract) | !file.exists(area_extra) | !file.exists(micro_extract)) {
...
the US is affected by the teperature and whether I see any evidence of my
...
than our IPUMS Terra, which which pulls from our microdata projects, but I use
...
- Raster: TEMPAVGFEB (mean, also year-specific and at lowest geogrpahic level available)
...
can use the raster pacakge's built in `plot()` method.
...
title("Average temp (deg C) in Feb, 1950-2010 (US raster)")
```
...
different sources (like rasterized microdata vs. the temperature data)
...
title("Percent of population ages 65-69 in 2010 (raseterized area)")
```
...
One solution is touse the `resample()` function from the raster package, which will use 
...
# Better projection for US Maps
terra_area <- st_transform(
  terra_area, 
  paste0(
...
terra_micro_shapes <- st_transform(
  terra_micro_shapes, 
  paste0(
    "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 ",
    "+x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs"
...
Now we'll create area level summaries (faceted by Age over 65)
...
# A true facet doesn't work well here because  the color scale
# is so different between the age groups so make 2 separate  
# maps instead
ggplot(
  ...
  geom_sf(linetype = "blank") + 
  ggtitle("65 and older: percent moved from another state in last year")

ggplot(
  graph_data %>% filter(age65plus = "Under 65"), 
  aes(fill = moved_states)
) + 
  geom_sf(linetype = "blank") + 
  ggtitle("Under 65: percent moved from another state in last year")
```
