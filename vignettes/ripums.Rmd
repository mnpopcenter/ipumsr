...
**For microdata projects (all projects except NHGIS or TerraPop)**, once you 
...
**For NHGIS & Terrapop**, download the table data as a csv, and, if you want 
...
in this vignette. To install, run the following code:
```{r, eval = FALSE}
...
to the actual data using the functions `read_ddi()` and `read_ipums_codebook()`. This
...
cps_data <- read_ipums_micro(cf, vars = c("YEAR", "AGE"), verbose = FALSE)

cps_data <- read_ipums_micro(cf, vars = c(YEAR, AGE), verbose = FALSE)

cps_data <- read_ipums_micro(cf, vars = c(one_of("YEAR"), starts_with("AG")), verbose = FALSE)
