...
the version on CRAN. If you are a beta tester, you can install the development 
...
Since we're installing from GitHub and building the package locally, we're also
installing ipumsexamples, and we've set `build_vignettes` and `dependencies` to
`TRUE`, to make sure that you have all the necessary example data and packages
to build the vignettes. Once the ipumsr API functionality is on CRAN, you will
get pre-built vignettes with `install.packages("ipumsr")`.
...
the IPUMS website. ipumsr includes functions that help R users interact with the
extract API from their R session.
...
```{r}
clone_of_usa_extract_33 <- define_extract_from_json("usa_extract_33.json")
