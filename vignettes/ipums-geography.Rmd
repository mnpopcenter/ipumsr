...
this vignette, there are some resources for learning more.
...
geographies that share area for different time periods and other methods).
These boundaries are called harmonized. 
- Level - The specificity of the data (for example states and
...
not (for example zip codes in the US may not fit within county boundaries).
...
for cooking in households with children aged 0-5 in Colobmia, 
Ecuador and Peru. 
...
give them the same structure. When combining non-harmonized geographies or
across diferent geographic levels, the variable names will not be the same.
...
We can investigate this join failure using hte `join_failures` command.
```{r}
join_failures(ipumsi)
```

In this case, it appears that the shape file includes Lake Titicaca for mapping
purposes, even though noone lives in the lake. We will just leave this boundary
out of our dataset.
...
- The CRAN website has a great tutorial called ["Introduction to visualizaing 
...
provides of the history behind the sf packge.
