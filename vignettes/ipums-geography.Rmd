...
IPUMS USA can be divided into eight steps:
...
4. Transform microdata as needed
...
### 5) Summarize microdata by geographic level
Microdata has one observation per person. To make a map we usually want to
summarize the data by geographic unit. In this case, because we have data at 
three time points for Colombia, we summarize by geographic unit and year.

This example is further complicated because we are using both harmonized and
year-specific geographic units, so the data currently has multiple variables
representing the geographic ID. Now is a good chance to combine them into one.

Here we create a single variable that combines the geographic ID from the 3 countries
and then calculate the percent of all children who live in a household
...
file. However, this can also happen when there is a geographic unit in the shape
files for which there are no microdata observations, or there is a geographic
unit in the microdata for which no shape file is available, perhaps because it
is too small.
...
Analysis](https://cran.r-project.org/web/views/Spatial.html)
