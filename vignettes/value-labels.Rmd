...
the IPUMS extract engine, but isn't that useful because this data set only has respondents 
from ASEC in March)
```{r}
...
```
The top-codes (which are only available on the CPS website) indicate that the values
