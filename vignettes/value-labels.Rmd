...
printing out a data.frame (like the "[Wisc~" on STATEFIP below, which stands for
Wisconsin). If your console supports colorful output, then the labels will be in 
a lighter grey than the values. (As a quick aside, you can turn off this printing
behavior by setting the option `options("ipumsr.show_pillar_labels" = FALSE)`)

However, as you can see, there often isn't enough space to see the labels when printing
out a full data.frame. So, there are a few options to see the labels:
```{r}
