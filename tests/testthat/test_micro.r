...
test_that(
  "Can read Rectangular .dat.gz", {
    cps <- read_ipums_micro(
      ipumsimport_example("cps_00006.xml"),
      data_file = ipumsimport_example("cps_00006.dat.gz"),
      verbose = FALSE
    )
...
test_that(
  "Can read Rectangular .csv.gz", {
    cps <- read_ipums_micro(
      ipumsimport_example("cps_00006.xml"),
      data_file = ipumsimport_example("cps_00006.csv.gz"),
      verbose = FALSE
    )
...
test_that(
  "Can read Hierarchical into nested format", {
    ...
    expect_equal(nrow(cps), rows_h)
    expect_equal(ncol(cps), vars_h + 1)
    expect_equal(attr(cps[["YEAR"]], "label"), YEAR_label)
    expect_equal(attr(cps[["YEAR"]], "var_desc"), YEAR_var_desc)
    expect_equal(attr(cps[["STATEFIP"]], "labels")[1:2], STATEFIP_val_labels)
    expect_equal(class(cps[["P"]]), "list")
  })
